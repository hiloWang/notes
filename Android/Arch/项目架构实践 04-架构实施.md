...
### BaseActivity 和 BaseFragment 的封装
...
- Presenter 与 View 的方法命名一定要 **规范统一**。
- V 和 M 不存在相互的感知，也就是不直接交互，P 作为 M 和 V 交互的桥梁。
- View 负责视图渲染和 UI 逻辑处理。当 UI 事件触发时，调用 Presenter 来处理，Presenter 收到 View 事件后调用 Model 获取（或提交）数据进行业务调度，并根据业务的调度状态来调用对应的 View 接口的方法，以展示不同视图。
...
- 原则上 Presenter 的实现中不应该依赖 Android SDK 中的类，以便于 Presenter 可以直接在 JVM 上进行单元测试。
...
MVP 中的 Presenter 的弊端 ，P 要做的事太多，不仅要从下层获取数据，还要调度 View 来控制 UI 的展示。何不让 View 自己做哪些事呢？
...
- MVVM 的重点在于 V 如何与 M 达成绑定，而绑定关系有：单向绑定和双向绑定。
- V 中始终是持有 VM 的引用的，所以 V 可以直接向VM发送命令，这也可以理解为 V 的行为绑定到了 VM，或者 VM 可以提供方法由 V 来调用。
- 使用 Kotlin 的 `Delegates.observable()` 代理属性可以方便地监听属性的修改。
- VM 如果将数据绑定到 V 呢？首先 VM 并不持有 V 的应用，它只能修改数据，而数据被修改之后自动触发 V 层响应，方式有：
