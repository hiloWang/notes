...
处理了崩溃，我们还会经常遇到 ANR（Application Not Responding，程序没有响应）这个问题。出现 ANR 的时候，系统还会弹出对话框打断用户的操作，这是用户非常不能忍受的。这又带来另外一个问题，我们怎么去发现应用中的 ANR 异常呢？总结一下，通常有两种做法。
...
1. 介绍了 Android 的两种崩溃，重点介绍了 Native 崩溃的捕获流程和一些难点。
...
./breakpad/src/src/processor/minidump_stackwalk a9663bf9-2eff-40ca-2346cf9d-f30b238d.dmp > crashLog.txt

# 符号解析：可以使用 ndk 中提供的 addr2line 来根据地址进行一个符号反解的过程
...
# 输入命令后进入交互，输入地址，addr2line 会输出代码行号
0x550
