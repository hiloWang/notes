...
块级元素和行内元素都可以浮动，**当一个行内元素浮动以后将会自动变为一 个块级元素**， 当一个块级元素浮动以后，宽度会变为默认由内容撑开，所以当漂浮一个块级元素时我们都会为其指定一个宽度。
...
-  浮动元素默认会变为块元素，即使设置 `display:inline` 以后其依然是个 块元素。
...
默认情况下没有浮动子元素可以撑起父元素，但是如果是浮动，那么**一个容器如果没有指定高度，就不能给自己的浮动的孩子撑起一个区域。**或者说不能被自己浮动的孩子撑起来。
